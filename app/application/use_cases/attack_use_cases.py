...
class ListAttacksUseCase:
    ...
    async def execute(self, tactical_game_id: Optional[str] = None,
...
class UpdateAttackUseCase:
    ...
    async def execute(self, attack_id: str, update_data: Dict[str, Any]) -> Optional[Attack]:
...
class ApplyAttackResultsUseCase:
    ...
    async def execute(self, attack_id: str, label: str, hit_points: int, 
                     criticals: List[Critical] = None) -> Optional[Attack]:
