...
class AttackDomainService:
    ...
    async def create_attack(self, attack: Attack) -> Attack:
        ...
        # Business rule: Attack must have a unique ID
        if await self._attack_repository.exists(attack.id):
            raise ValueError(f"Attack with ID {attack.id} already exists")
        
        # Business rule: New attacks start as pending
    ...
    async def execute_attack_roll(self, attack_id: str, roll_value: int) -> Optional[Attack]:
        ...
        if not attack.is_pending():
            raise ValueError(f"Cannot roll for attack {attack_id}: attack is not pending")
    ...
    async def apply_attack_results(self, attack_id: str, label: str, 
                                 hit_points: int, criticals: list[Critical] = None) -> Optional[Attack]:
        ...
        if not attack.roll:
            raise ValueError(f"Cannot apply results to attack {attack_id}: no roll executed")
    ...
    def calculate_attack_result(self, roll_value: int, attack_bonus: int = 0) -> tuple[str, int]:
        """
